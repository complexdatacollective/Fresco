...
test.describe.parallel('Interview Export - parallel', () => {
  ...
  test('should show export dropdown menu with options', async ({ page }) => {
    ...
    await expect(
      page.getByRole('menuitem', { name: /export all unexported interviews/i }),
    ).toBeVisible();
...
test.describe.serial('Interview Export - serial', () => {
  ...
  test('should show export unexported interviews option', async ({
    page,
    database,
  }) => {
    const cleanup = await database.isolate('export-unexported');
    await page.goto('/dashboard/interviews', { waitUntil: 'domcontentloaded' });
    ...
    try {
      ...
      // "Export all unexported interviews" should be present
      const unexportedOption = page.getByRole('menuitem', {
        name: /export all unexported interviews/i,
      });
      await expect(unexportedOption).toBeVisible();
