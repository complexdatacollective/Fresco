 ...
 * for now.
 */
const useSpeech = (text, lang = null) => {
  const [navigatorLanguage, setNavigatorLanguage] = useState(lang);
  ...
  const voiceForLanguage = useMemo(() => voices.find(
    ...
    (voice) => voice.lang.toLowerCase() === navigatorLanguage?.toLowerCase(),
  ), [navigatorLanguage, voices]);
