...
const mockEdges = [
  generateEntity(
    { stringVariable: 'string', booleanVariable: true, numberVariable: 6 },
    { from: 1, to: 2 },
    'edge',
    'friend',
  ),
  generateEntity(
    { stringVariable: 'not', booleanVariable: false, numberVariable: 4 },
    { from: 2, to: 3 },
    'edge',
    'friend',
  ),
...
const getMatchedNames = (matches) =>
  matches.map((node) => node[entityAttributesProperty].name);

const runRuleHelper = (ruleConfig) => {
  const rule = getRuleFunction(ruleConfig);
...
describe('rules', () => {
  describe('alter rules', () => {
    describe('faulty rules', () => {
      it('correctly handles missing attribute (EXACTLY)', () => {
        const ruleConfig = generateRuleConfig('node', {
          type: 'person',
        ...
        const { nodes } = runRuleHelper(ruleConfig);
  ...
  describe('edge rules', () => {
    it('EXISTS', () => {
      const ruleConfig = generateRuleConfig('edge', {
        type: 'friend',
        operator: 'EXISTS',
      });
