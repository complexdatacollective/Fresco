...
describe('query', () => {
  ...
  describe('rule types', () => {
    ...
    const trueNodeRule1 = generateRuleConfig('node', {
      type: 'person',
      operator: 'LESS_THAN',
      attribute: 'age',
      value: 20,
    ...
    const falseNodeRule1 = generateRuleConfig('node', {
      type: 'person',
      operator: 'LESS_THAN',
      attribute: 'age',
      value: 0,
    ...
    describe('alter rules', () => {
      it('an alter rule passes if ANY node matches the rule', () => {
        ...
        const trueNodeRule2 = generateRuleConfig('node', {
          type: 'person',
          operator: 'EXACTLY',
          attribute: 'likesFish',
          value: false,
    ...
    describe('edge rules', () => {
      ...
      it('AND edge rules mean a SINGLE NODE must match BOTH edge rules (example pass)', () => {
        const andQuery = getQuery({
          join: 'AND',
          rules: [
            generateRuleConfig('node', { type: 'person', operator: 'EXISTS' }),
            generateRuleConfig('edge', { type: 'friend', operator: 'EXISTS' }),
    ...
    describe('combining rule types', () => {
      const trueEdgeRule = generateRuleConfig('edge', {
        type: 'friend',
        operator: 'EXISTS',
      });
