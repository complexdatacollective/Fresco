...
describe('validateSchema', () => {
  ...
  it('ensures protocol schema is defined', () => {
    const expectedError = new Error(
      "Provided schema version '-99' is not defined",
    );

    expect(validateSchema(getProtocol({ schemaVersion: -99 }))).toEqual([
      expectedError,
    ]);
  });
