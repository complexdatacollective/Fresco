...
const validateLogic = (protocol: Protocol) => {
  ...
  v.addValidation(
    ...
    (nodeType: NcNode) =>
      `node displayVariable "${nodeType.displayVariable}" did not match unknown node variable`,
  );
  ...
  v.addValidationSequence(
    'stages[].subject',
    [
      (subject: StageSubject, _: unknown, keypath: Array<string>) => {
        const stage = get(protocol, `${keypath[1]}${keypath[2]}`) as Stage;
    ...
    [
      ...
      (subject: StageSubject[]) =>
        `Duplicate subject type "${duplicateInArray(
          subject.map(getSubjectTypeName),
        )}"`,
    ],
    [
      (subject: StageSubject) => {
  ...
  // Needs to:
  // 1. Check that unknown variables referenced by a validation exist in the codebook
  // 2. Check that validation is not applied on a variable that is on an inappropriate
  //      entity type.
  v.addValidation(
    'codebook.ego.variables.*.validation',
    // First, check that unique is not applied on unknown ego variables
    (validation: VariableValidation) =>
      !Object.keys(validation).includes('unique'),
    (_: unknown, __: unknown, keypath: Array<string>) =>
  ...
  v.addValidation(
    ...
    (validations: VariableValidation, _: unknown, keypath: Array<string>) => {
      ...
      const variablesForType = get(protocol, path, {}) as Record<
        string,
        VariableDefinition
      >;
      ...
      return typesToCheck.every((type) => {
        const variable = get(validations, type) as string;
        return Object.keys(variablesForType).includes(variable);
      });
    },
    (validation: VariableValidation, _: unknown, keypath: Array<string>) => {
      const subject = {
        entity: keypath[2],
        type: keypath[3],
      };
      return `Validation configuration for the variable "${getVariableNameFromID(
        ...
        subject,
        keypath[5],
      )}" on the ${subject.entity} type "${getSubjectTypeName(
        codebook,
        subject,
      )}" is invalid! The variable "${
        Object.values(validation)[0]
      }" referenced by the validation does not exist in the codebook for this type.`;
    },
  );
  ...
  v.addValidationSequence(
    ...
    [
      ...
      (rule: FilterRule) =>
        `"${rule.options.attribute}" is not a valid variable ID`,
    ],
  );
  ...
  v.addValidation(
    ...
    (panels: object[]) =>
      `Panels contain duplicate ID "${duplicateId(panels)}"`,
  );
  ...
  v.addValidation(
    ...
    (prompts: Prompt[]) =>
      `Prompts contain duplicate ID "${duplicateId(prompts)}"`,
  );
  ...
  v.addValidation(
    ...
    (items: ItemDefinition[]) =>
      `Items contain duplicate ID "${duplicateId(items)}"`,
  );

  v.addValidation(
    'codebook.*.*.variables',
    (variableMap: Record<string, VariableDefinition>) =>
      !duplicateInArray(getVariableNames(variableMap)),
    (variableMap: Record<string, VariableDefinition>) =>
  ...
  v.addValidation(
    ...
    (variable: string, subject: StageSubject) =>
      getVariablesForSubject(codebook, subject)[variable],
    (variable: string, subject: StageSubject) =>
      `"${variable}" not defined in codebook[${subject.entity}][${subject.type}].variables`,
  );
  ...
  v.addValidation(
    ...
    (otherVariable: string, subject: StageSubject) =>
  ...
  v.addValidation(
    ...
    (additionalAttributes: AdditionalAttributes, subject: StageSubject) =>
