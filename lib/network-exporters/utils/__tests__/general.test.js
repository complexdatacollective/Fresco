...
const {
  ExportError,
  ErrorMessages,
} = require('../../consts/errors/ExportError');
const {
...
describe('verifySessionVariables', () => {
  it('throws an error if the session variables are missing', () => {
    const session = {};
    expect(() => verifySessionVariables(session)).toThrow(
      new ExportError(ErrorMessages.MissingParameters),
    );
  });
});
...
describe('makeFormats', () => {
  ...
  it('throws an error when an unsupported format is referenced', () => {
    expect(() => makeFormats(['unsupported'])).toThrow(
      new ExportError(ErrorMessages.InvalidFormat),
    );
