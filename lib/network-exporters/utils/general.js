...
const getFilePrefix = (session, protocol, unifyNetworks) => {
  ...
  return `${sanitizeFilename(caseAsString)}_${
...
const getFileExportListFromFormats = (formats) => {
...
const makeFormats = (userFormats) => {
  ...
  if (Array.isArray(userFormats)) {
    return userFormats.reduce((acc, format) => {
      ...
      return {
        ...acc,
        [format]: Object.keys(SUPPORTED_FORMATS[format].options).reduce(
          (acc2, option) => ({
            ...acc2,
  ...
  if (typeof userFormats === 'object') {
    return Object.keys(userFormats).reduce((acc, format) => {
      ...
      const options = Object.keys(defaultFormatOptions).reduce(
        (acc2, option) => {
          if (has(userFormatOptions, option)) {
            if (
              !validateUserFormatOptionType(
                format,
                option,
                userFormatOptions[option],
              )
            ) {
              // eslint-disable-next-line no-console
              console.warn(
                `${option} is not a valid option for ${format}. Ignoring.`,
              );
              return {
                ...acc2,
