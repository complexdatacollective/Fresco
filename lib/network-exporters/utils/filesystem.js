...
//   if (environment === environments.ELECTRON) {
//     const fse = dynamic(() => import('fs-extra'));
...
//   throw new Error(
//     `getFileNativePath() not available on platform ${environment}`,
//   );
// });
...
const copy = (oldPath, newPath) => {
  const fse = dynamic(() => import('fs-extra'));
  return fse.copy(oldPath, newPath);
};
