import {
  VariableTypeEnum as VariableType,
  caseProperty,
...
import { findKey, groupBy } from 'es-toolkit';
import { includes } from 'es-toolkit/compat';
import jsSHA from 'jssha/dist/sha1';
import { getAttributePropertyFromCodebook } from '~/lib/network-exporters/utils/general';
import { getEntityAttributes } from '~/utils/general';
import { createDataElement, formatXml, getGraphMLTypeForKey } from './helpers';
...
function* graphMLGenerator(network, codebook, exportOptions) {
  ...
  if (exportOptions.globalOptions.unifyNetworks) {
    ...
    const groupedNetwork = {
      nodes: groupBy(network.nodes, (n) => n[sessionProperty]),
      edges: groupBy(network.edges, (e) => e[sessionProperty]),
    };
