...
const generateEgoDataElements = (
  ...
  const { screenLayoutWidth, screenLayoutHeight, useScreenLayoutCoordinates } =
    exportSettings;
...
const generateDataElements = (
  ...
  const { useScreenLayoutCoordinates, screenLayoutWidth, screenLayoutHeight } =
    exportSettings;
...
function* graphMLGenerator(network, codebook, exportSettings) {
  ...
  const generateEgoKeys = (ego) =>
    generateKeyElements(xmlDoc, [ego], 'ego', [], codebook);
  ...
  const generateNodeElements = (nodes) =>
    generateDataElements(xmlDoc, nodes, 'node', [], codebook, exportSettings);

  const generateEdgeElements = (edges) =>
    generateDataElements(xmlDoc, edges, 'edge', [], codebook, exportSettings);

  const generateEgoElements = (ego) =>
    generateEgoDataElements(xmlDoc, ego, [], codebook, exportSettings);
