...
import { mockCodebook, mockExportOptions, mockNetwork, mockNetwork2, processMockNetworks } from '../../csv/__tests__/mockObjects';
...
const buildXML = (...args) => {
  let xmlString = '';
  for (const chunk of graphMLGenerator(...args)) {
    // eslint-disable-line no-restricted-syntax, no-unused-vars, max-len
    xmlString += chunk;
  }

  const parser = new DOMParser();
  const result = parser.parseFromString(xmlString, MIME_TYPE.XML_TEXT);
  return result;
};
...
describe('buildGraphML', () => {
  ...
  it('infers int types', () => {
    ...
    expect(xml.getElementById('mock-uuid-2').getAttribute('attr.type')).toEqual(
      'int',
    );
  });
