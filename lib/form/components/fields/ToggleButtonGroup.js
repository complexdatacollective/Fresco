...
const ToggleButtonGroup = ({ options = [], className, label, fieldLabel }) => {
  ...
  const value = useMemo(() => fieldContext.state.value || [], [fieldContext.state.value]);
  ...
  const renderOption = useCallback((option, index) => {
    ...
    return (
      <label
        key={index}
        className={cx('form-field-togglebutton', `form-field-togglebutton-cat-color-seq-${index}`, 'form-field-togglebutton-group__option')}
  ...
  const classNames = cx(
    ...
    className,
    {
      'form-field-togglebutton-group--has-error': !fieldContext.state.meta.isValid && fieldContext.state.meta.isTouched && fieldContext.state.meta.errors?.[0],
    },
  );
