...
export default function OtherChildrenDetailStep() {
  const { noChildrenWithOther } = useFormValue(['noChildrenWithOther']);
  const childCount = Number(noChildrenWithOther ?? 0);
    ...
    <div className="flex flex-col gap-6 pt-4">
      {Array.from({ length: childCount }, (_, i) => (
        <Surface key={i} level={1} spacing="sm">
          ...
          <PersonFields nameToggle={false} namespace={`otherChild[${i}]`} />
        </Surface>
      ))}
    </div>
  );
