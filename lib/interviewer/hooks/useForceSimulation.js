...
const useForceSimulation = (listener = () => undefined) => {
  ...
  const initialize = useCallback((network = {}, options = {}) => {
    ...
    worker.current = new Worker(new URL('./forceSimulation.worker.js', import.meta.url))
    ...
    setIsRunning(false);
  }, [setIsRunning, listener, calculateLayoutCoords, calculateRelativeCoords, autoZoom]);
