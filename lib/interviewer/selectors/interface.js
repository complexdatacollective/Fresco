...
const makeGetNodeVariables = () => getNodeVariables;

const getEdgeVariables = createSelector(
  getProtocolCodebook,
  getSubjectType,
  (codebook, edgeType) => {
    const edgeInfo = codebook.edge;
    return edgeInfo?.[edgeType]?.variables || {};
  },
);

const makeGetEdgeVariables = () => getEdgeVariables;

const getVariableOptions = createSelector(
  (state, props) => {
    if (props?.subject?.entity === 'edge') {
      return getEdgeVariables(state, props);
    }

    return getNodeVariables(state, props);
  },
  (state, { variable }) => variable,
  (variables, variable) => {
    return variables[variable]?.options || []
  },
);
...
const makeNetworkEntitiesForType = () => getNetworkEntitiesForType;
...
const makeNetworkNodesForPrompt = () => getNetworkNodesForPrompt;
...
const makeNetworkNodesForOtherPrompts = () =>
