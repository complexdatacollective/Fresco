...
import { propStageId } from './prop';
import { getPromptId, getStageSubject, getSubjectType } from './session';
...
const getIDs = createSelector(propStageId, getPromptId, (stageId, promptId) => {
  return {
    stageId,
    promptId,
  };
...
export const getNodeIconName = createSelector(
  ...
  (codebook, nodeType) => {
    ...
    return (
      (nodeInfo && nodeInfo[nodeType] && nodeInfo[nodeType].iconVariant) ||
      'add-a-person'
    );
  },
);
