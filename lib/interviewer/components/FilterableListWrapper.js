...
class FilterableListWrapper extends Component {
  ...
  getFilteredList = (list) => {
    ...
    const filteredList = list.filter((node) => {
      ...
      return nodeDetails.some(
        (item) =>
          item && item.toString().toLowerCase().includes(lowerCaseFilterValue),
      );
    });
  ...
  render() {
    const { sortFields, ListComponent, listComponentProps, items } = this.props;
    const { activeSortOrder, filterValue } = this.state;
            ...
            {sortFields && sortFields.length > 0 && (
              <>
                ...
                {sortFields.map((sortField) => (
                  <Button
                    color={
                      activeSortOrder.property === sortField.variable
                        ? 'primary'
                        : 'white'
                    }
...
FilterableListWrapper.propTypes = {
  ...
  listComponentProps: PropTypes.shape({}).isRequired,
};
