...
const Search = ({
  ...
  if (isLoading) {
    return (
      <Loading className="search__loading" message="Loading roster data..." />
    );
  }
  ...
  const headers = [SearchPrompt, SelectPrompt].map((prompt, i) => {
    let hiddenClass = '';
    if (
      (prompt === SearchPrompt && hasSearchTerm) ||
    ...
    return (
      <h2
        className={`${HeaderClass} ${hiddenClass}`}
        key={`${HeaderClass}${i}`}
      >
        {prompt}
      </h2>
  ...
  return (
    <>
      ...
      <SearchTransition className={searchClasses} in={isOpen}>
        <form onSubmit={formSubmitHandler}>
          <Icon
            name="close"
            className="menu__cross search__close-button"
            onClick={onClose}
          />
