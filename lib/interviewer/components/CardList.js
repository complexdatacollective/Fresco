...
class CardList extends Component {
  ...
  UNSAFE_componentWillMount() {
    this.columns = calculateRequiredColumns(
      window.innerWidth,
      window.innerHeight,
    );
  }
  ...
  rowRenderer = ({
    ...
    const { details, label, onItemClick, isItemSelected, getKey } = this.props;
    ...
    return (
      <CellMeasurer
        ...
        parent={parent}
      >
        <div className="card-list__row" style={{ ...style, display: 'flex' }}>
          {nodes.map((node, column) => {
            if (!node) {
              return (
                <span
                  className="card-list__content"
                  key={`column-space-${column}`}
                />
              );
            }
            return (
