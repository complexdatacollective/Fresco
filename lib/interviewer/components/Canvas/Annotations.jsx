...
class Annotations extends Component {
  ...
  componentDidMount() {
    const nodeListRoot =
      document.getElementsByClassName('node-layout').length > 0
  ...
  onDragStart = (mouseEvent) => {
    const { lines, linesShowing, activeLines } = this.state;
  ...
  onDragEnd = () => {
    ...
    this.removeLineTimers.push(
      setTimeout(this.fadeLines.bind(null, lines.length - 1), 1000),
    );
  };
  ...
  handleLineGone = (position) => {
    const { linesShowing, activeLines } = this.state;
  ...
  unfreeze = () => {
    ...
    linesShowing.forEach((showing, index) => {
      if (showing) {
        this.removeLineTimers.push(
          setTimeout(this.fadeLines.bind(null, index), 1000),
        );
      }
    });
  };
  ...
  render() {
    const { lines, linesShowing, linesToFade, isDrawing } = this.state;
    const { isFrozen } = this.props;
