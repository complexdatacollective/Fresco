...
} from '~/lib/ui/utils/CSSVariables';

const getCssProp = (computedStyle, name) =>
...
class Search extends Component {
  ...
  runEnterTimeline(el) {
    ...
    const collapsedSizePx = getPxValue(
      getCssProp(elStyle, '--collapsed-square-size-js'),
    );
    const expandedSizePx = getPxValue(getCssProp(elStyle, 'width'));
    const borderRadiusPx = getPxValue(
      getCssProp(elStyle, '--nc-border-radius'),
    );
    ...
    const rightCollapsed = getPxValue(
      getCssProp(elStyle, '--right-collapsed-js'),
    );
    ...
    this.runningAnimation = anime
      .timeline(this.TimelineOpts)
      .add({
        targets: el,
        ...wrapperAnimation,
  ...
  runExitTimeline(el) {
    ...
    if (rightCollapsed && rightExpanded) {
      inverseWrapperAnimation.right = [
        getPxValue(rightExpanded),
        getPxValue(rightCollapsed),
      ];
    }
