...
import { Search } from '~/lib/ui/components/Fields';
...
export const getFilteredList = (items, filterTerm, searchPropertyPath) => {
  ...
  return items.filter((item) => {
    const itemAttributes = searchPropertyPath
      ? Object.values(get(item, searchPropertyPath, {}))
      : Object.values(item);
    ...
    return itemAttributes.some(
      (property) =>
        property &&
        property.toString().toLowerCase().includes(normalizedFilterTerm),
    );
  });
};
...
const NewFilterableListWrapper = (props) => {
  ...
  const filteredItems = onFilterChange
    ? items
    : getFilteredList(items, filterTerm, searchPropertyPath);
  ...
  return (
    <div className="new-filterable-list">
              ...
              {sortableProperties.map((sortField, index) => (
                <div
                  ...
                  className={`filter-button ${
                    sortRule === index ? 'filter-button--active' : ''
                  }`}
      ...
      >
        {loading ? (
          <motion.div className="loading-state" layout key="loader">
            <Spinner small />
            <h4>Loading...</h4>
          </motion.div>
        ) : (
