...
const NodeLayout = forwardRef(function NodeLayout(props, forwardedRef) {
  ...
  useEffect(() => {
    console.log('updateNodePositions changed');
    animationFrameRef.current = requestAnimationFrame(updateNodePositions);
    return () => cancelAnimationFrame(animationFrameRef.current);
  }, [updateNodePositions]);

  useEffect(() => {
    console.log('screen changed, initializing');
    if (!innerRef.current) {
      return;
    }
  ...
  useEffect(() => {
    console.log('nodes.lengh changed generating layoutEls');
    if (
  ...
  const handleDrag = useCallback((uuid, index, delta, isDragEnd) => {
    console.log('handledrag', uuid, index, delta, isDragEnd);
    if (!allowPositioning) {
      return;
    }
