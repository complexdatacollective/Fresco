...
export const LayoutProvider = ({
  ...
  console.log('Window from LayoutContext: ', window);
  ...
  const updateNetworkInStore = useCallback(() => {
    ...
    nodes.forEach((node, index) => {
      ...
      dispatch(
        sessionActions.updateNode(node[entityPrimaryKeyProperty], undefined, {
          [layoutVariable]: { x: clamp(x, 0, 1), y: clamp(y, 0, 1) },
        }),
      );
    });
  }, [dispatch, nodes, layout, twoMode, forceSimulation]);
