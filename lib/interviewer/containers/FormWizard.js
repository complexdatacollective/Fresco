...
const NavigationButton = ({ icon, left, right, onClickHandler }) => {
  const classes = cx('wizard-navigation-button', {
      ...
      <div className="form-wizard">
        <div className="form-wizard__previous">
          {fieldIndex !== 0 && (
            <NavigationButton
              left
              onClickHandler={this.previousField}
              icon="chevron-left"
            />
          )}
        </div>
        <Form {...formProps} submitButton={nextButton} />
      </div>
    );
  }
}
