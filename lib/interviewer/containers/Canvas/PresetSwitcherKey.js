  ...
  renderHighlightLabel = (highlight, index) => {
    ...
    return (
      <Radio
        ...
        label={highlight}
        data-testid={`attribute-item-${index}`}
      />
    );
  };
        ...
        <div className="preset-switcher-key__content">
          {!isEmpty(highlightLabels) && (
            <Accordion
              ...
              data-testid="attributes-accordion"
            >
          ...
          {!isEmpty(edges) && (
            <Accordion
              label="Links"
              onAccordionToggle={toggleEdges}
              data-testid="links-accordion"
            >
              {edges.map((edge, index) => (
                <div
                  className="accordion-item"
                  key={index}
                  data-testid={`edge-label-${index}`}
                >
          ...
          {!isEmpty(convexOptions) && (
            <Accordion
              label="Groups"
              onAccordionToggle={toggleHulls}
              data-testid="groups-accordion"
            >
              {convexOptions.map((option, index) => (
                <div
                  className="accordion-item"
                  key={index}
                  data-testid={`group-label-${index}`}
                >
                  <Icon
                    name="contexts"
