...
class PresetSwitcher extends PureComponent {
  ...
  render() {
    ...
    const freezeClasses = cx('preset-switcher__freeze', {
    ...
    return (
      <div className="preset-switcher">
        ...
        <div
          className={cx('preset-switcher__reset-button', {
            'preset-switcher__reset-button--show': shouldShowResetButton,
          })}
          onClick={onResetInteractions}
          data-testid="reset-narrative-button"
        >
          <Icon name="reset" />
        </div>
        <div
          className="preset-switcher__label"
          data-testid="preset-switcher-label"
        >
          <div style={{ width: '100%' }} onClick={this.toggleOpen}>
            <h4>
              <HoverMarquee>{currentActivePreset.label}</HoverMarquee>
            </h4>
          </div>
        </div>
        <div
          className={cx(
            ...
            {
              'preset-switcher__navigation--disabled':
                activePreset + 1 === presets.length,
            },
          )}
          onClick={() => onChangePreset(activePreset + 1)}
        >
...
export default compose(DropObstacle)(PresetSwitcher);
