...
const useGridSizer = (
  ...
  const rowHeight = useCallback(
    (rowIndex) => {
      ...
      const height = items.slice(start, end).reduce((acc, item) => {
        hiddenSizingEl.innerHTML = renderToString(
          <ItemComponent {...item.props} />,
        );
        return hiddenSizingEl.clientHeight > acc
    ...
    [hiddenSizingEl, items, columnWidth, ItemComponent, columns, defaultHeight],
  );
