...
const getRowRenderer = (ItemComponent, DragComponent, allowDragging) => {
  const GetRowRenderContent = ({ index, style }) => {
    ...
    const { nodeRef, preview: dragPreview } = useDragSource({
...
const HyperList = ({
  ...
  const enhancedClassNames = cx(
    classNames,
    { 'hyper-list--drag': monitorState.willAccept },
    { 'hyper-list--hover': monitorState.willAccept && monitorState.isOver },
  );
