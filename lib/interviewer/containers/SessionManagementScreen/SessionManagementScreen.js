...
import { Text } from '~/lib/ui/components/Fields';
import { Modal } from '~/lib/ui/components/Modal';
import Scroller from '~/lib/ui/components/Scroller';
import ExportSprite from '~/lib/ui/components/Sprites/ExportSprite';
...
const DataExportScreen = ({ show, onClose }) => {
  ...
  const deleteSession = (id) => dispatch(sessionActions.removeSession(id));
  ...
  const handleDeleteSessions = () => {
    openDialog({
      type: 'Warning',
      title: `Delete ${selectedSessions.length} Interview Session${
  ...
  const renderFooter = () => {
    ...
    return (
      ...
      >
        ...
        <Button color="primary" onClick={() => exportSessions(false)}>
