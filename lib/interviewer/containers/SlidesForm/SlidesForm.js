...
const SlidesForm = (props) => {
  ...
  const isLeftStage = useRef(false);
  ...
  const beforeNext = (direction) => {
    // this is a hack to prevent beforeNext from being called
    // after the stage has been left
    if (isLeftStage.current) {
      isLeftStage.current = false;
      return true;
    }

    // Leave the stage if there are no items
    if (items.length === 0) {
      isLeftStage.current = true;
      return true;
    }
