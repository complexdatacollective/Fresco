...
const OrdinalBins = ({ bins, prompt, stage, activePromptVariable, updateNode }) => {
  ...
  const backgroundColor = useCallback(() => color(getCSSVariableAsString('--nc-background')), []);

  const calculateAccentColor = useCallback((index, missingValue) => {
    if (missingValue) {
  ...
  const calculatePanelColor = useCallback((index, missingValue) => {
    if (missingValue) {
      return color(getCSSVariableAsString('--color-rich-black'))
  ...
  const calculatePanelHighlightColor = useCallback((missingValue) => {
    if (missingValue) {
  ...
  const renderOrdinalBin = useCallback((bin, index) => {
