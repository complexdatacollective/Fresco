...
import { getNetworkNodesForType, makeGetVariableOptions } from '../selectors/interface';
...
class OrdinalBins extends PureComponent {
  ...
  calculatePanelColor = (index, missingValue) => {
    ...
    const blendRatio = 1 / (bins.length) * index;
    return color(getCSSVariableAsString('--nc-panel-bg-muted')).mix(this.backgroundColor(), blendRatio).toString();
  };
  ...
  calculatePanelHighlightColor = (missingValue) => {
    ...
    return color(getCSSVariableAsString('--nc-panel-bg-muted')).mix(this.promptColor(), 0.2).toString();
  };
...
function mapDispatchToProps(dispatch) {
  return {
    updateNode: bindActionCreators(sessionActions.updateNode, dispatch),
  };
}
