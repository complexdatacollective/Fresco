...
import { makeNetworkNodesForPrompt, makeGetAdditionalAttributes, makeGetStageNodeCount, getStageNodeCount, getAdditionalAttributesSelector, getNetworkNodesForPrompt } from '../../selectors/interface';
import { makeGetPromptNodeModelData, makeGetNodeIconName, getPromptModelData as getPromptNodeModelData } from '../../selectors/name-generator';
...
const NameGenerator = (props) => {
  ...
  const stageNodeCount = useSelector((state) => getStageNodeCount(state, props)); // 1
  const newNodeAttributes = useSelector((state) => getAdditionalAttributesSelector(state, props)); // 2
  const newNodeModelData = useSelector((state) => getPromptNodeModelData(state, props)); // 3
  const nodesForPrompt = useSelector((state) => getNetworkNodesForPrompt(state, props)); // 4
