...
const NameGenerator = (props) => {
  ...
  const completeHandler = useRef(null);

  const { currentPrompt, isFirstPrompt, isLastPrompt } = usePrompts();
  ...
  const handleBeforeLeaving = useCallback((direction, destination) => {
    const isLeavingStage =
      (isFirstPrompt && direction === -1) || (isLastPrompt && direction === 1);
    ...
    return true;
  }, [isFirstPrompt, isLastPrompt, stageNodeCount, minNodes]);

  useEffect(() => {
    registerBeforeNext(handleBeforeLeaving);
  }, [handleBeforeLeaving, registerBeforeNext]);
  ...
  const handleDropNode = (item) => {
    ...
    if (has(node, 'promptIDs')) {
      addNodeToPrompt(node[entityPrimaryKeyProperty], currentPrompt.id, {
        ...newNodeAttributes,
      });
    } else {
