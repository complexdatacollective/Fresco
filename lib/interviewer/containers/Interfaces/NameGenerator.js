...
const NameGenerator = (props) => {
  ...
  const addNode = useCallback(
    (attributes) => dispatch(sessionActions.addNode(subject.type, attributes)),
    [dispatch, subject],
  );
  ...
  const addNodeToPrompt = useCallback(
    (...properties) => dispatch(sessionActions.addNodeToPrompt(...properties)),
    [dispatch],
  );

  const deleteNode = useCallback(
    (uid) => {
      dispatch(sessionActions.deleteNode(uid));
    },
    [dispatch],
  );
