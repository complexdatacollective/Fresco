...
const DyadCensus = ({
  ...
  const [hasEdge, setEdge, isTouched, isChanged] = useNetworkEdgeState(
    dispatch,
    edges,
    prompts.currentPrompt?.createEdge, // Type of edge to create
    pair,
    prompt.id,
    stageState,
    [stepsState.step],
...
const makeMapStateToProps = () => {
  ...
  const mapStateToProps = (state, props) => {
    ...
    const edgeColor = get(codebook, [
      'edge',
      props.prompt?.createEdge,
      'color',
    ]);
    const pairs = getPairs(nodes);
    const stageState = getStageIndex(state);
...
export default compose(connect(makeMapStateToProps))(DyadCensus);
