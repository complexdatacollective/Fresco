...
import { getStageState } from '~/lib/interviewer/selectors/session';
...
const useEdgeState = (
  ...
  const stageState = useSelector(getStageState);
  ...
  const setEdge = (value = true) => {
    ...
    console.log('newStageState', newStageState);

    if (addEdge) {
      dispatch(
        sessionActions.addEdge({ from: pair[0], to: pair[1], type: edgeType }),
      );
    } else if (removeEdge) {
      dispatch(
        sessionActions.removeEdge(existingEdge[entityPrimaryKeyProperty]),
      );
    }

    dispatch(sessionActions.updateStageState(newStageState));
  };
