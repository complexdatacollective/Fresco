...
import { has, omit, reduce } from 'lodash-es';
import { v4 as uuid } from 'uuid';
import { encryptData } from '../../containers/Interfaces/Anonymisation';
import { getPassphrase } from '../../utils/labelLogic';
...
const getReducer = (network) => (state = initialState, action = {}) => {
  switch (action.type) {
    case SET_SERVER_SESSION: {
      ...
      const { protocol: { id: protocolUID } } = action.payload;
      ...
      return {
        ...
        [id]: {
          ...
          network: action.payload.session.network ?? network(state.network, action),
...
const addNode = (modelData, attributeData = {}) => async (dispatch, getState) => {
  const { activeSessionId, sessions, installedProtocols } = getState();
  ...
  let secureAttributes = {};

  // Handle encryption of secure attributes by iterating through attribute data
  for (const [variableId, value] of Object.entries(attributeData)) {
    if (registryForType[variableId].encrypted) {
      const { _secureAttributes, data } = await encryptData(value, getPassphrase());
      attributeData[variableId] = data;
      secureAttributes[variableId] = _secureAttributes;
    }
  }

  console.log({ secureAttributes, attributeData });
