...
const ProgressMessages = {
  ...
  NoResponseMessage:
    'Server could not be reached at the address you provided. Check your networking settings on this device, and on the computer running Server and try again. Consult our documentation on pairing for detailed information on this topic.',
};
...
const apiMismatchError = (code, response) => {
  ...
  error.friendlyMessage =
    'The device does not match the server API version. Ensure that Interviewer and Server are running compatible versions.';
  error.code = code;
...
class ApiClient {
  ...
  addTrustedCert() {
    ...
    return Promise.reject(
      new Error('SSL connections to Server are not supported on this platform'),
    );
  }
  ...
  requestPairing() {
    ...
    return this.pairingClient
      .get('/devices/new', { cancelToken: this.cancelTokenSource.token })
      .then((resp) => resp.data)
      .then((json) => json.data)
      .catch(handleError);
  }
  ...
  downloadProtocol(path, destination) {
    ...
    return Promise.reject(
      new Error('Downloads not supported on this platform'),
    );
  }
  ...
  exportSession(sessionData) {
    const {
      sessionVariables: { sessionId, protocolUID },
    } = sessionData;
    ...
    return this.httpsClient
      .post(`/protocols/${protocolUID}/sessions`, payload, this.authHeader)
      .then((resp) => resp.data)
      .then((json) => json.data);
  }
  ...
  exportSessions(sessionList) {
    ...
    const exportPromise = sessionList
      .reduce(async (previousPromise, nextSession, index) => {
        ...
        return this.exportSession(nextSession)
          .then(() =>
            this.emit(
              'session-exported',
              sessionList[index].sessionVariables.sessionId,
            ),
          )
          .catch((error) => {
            ...
            if (responseError) {
              this.emit(
                'error',
                `${sessionList[index].sessionVariables.caseId}: ${responseError.message} - ${responseError.friendlyMessage}`,
              );
              return;
            }
            // Handle errors with the request
          ...
          .then(() => {
            if (!cancelled) {
              this.emit(
                'update',
                ProgressMessages.ExportSession(index + 1, sessionList.length),
              );
            }
            Promise.resolve();
          });
