...
/**
 * Hook to check if a specific key is selected.
 * More efficient than using the full SelectionManager when you only need
 * to check selection status.
 *
...
export function useIsSelected(key: Key): boolean {
  ...
  const disabledKeys = useCollectionStore<unknown, Set<Key>>(
...
/**
 * Hook to check if a specific key is focused.
 *
 * @param key - Key to check
...
/**
 * Hook to check if a specific key is disabled.
 *
 * @param key - Key to check
