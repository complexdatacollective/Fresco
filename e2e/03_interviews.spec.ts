...
test.describe('Interviews page', () => {
  ...
  test('should display completed interview', async ({ page }) => {
    ...
    await expect(page.getByText('Sample Protocol.netcanvas')).toBeVisible();
  });

  // export interview data
  test('should export interview data', async ({ page }) => {
    await page.getByTestId('export-interview-button').click();
    await page.getByTestId('export-all-button').click();
    await expect(page.getByText('confirm-export-dialog-title')).toBeVisible();
