...
const asIntName = (schemaName: string) => {
 ...
 * @returns 
 */
const generateModuleIndex = async (schemas: Array<string>, outputPath: string) => {
  ...
  const formatVersions = (baseSchemaName: string) => `  { version: ${asIntName(baseSchemaName)}, validator: ${asVariableName(baseSchemaName)} },`;
 ...
 * Compile 
 */
export const buildSchemas = async (sourcePath: string, outputPath: string) => {
...
export const buildSchema = async (schema: string, sourceDirectory: string, outputDirectory: string) => {
  ...
  let validatedSchema;
  try {
    validatedSchema = ajv.compile(schemaJson);
  } catch (e) {
